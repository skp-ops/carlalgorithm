...
# 总体的原则就是：当前高度小于等于栈顶高度，入栈，指针后移。 当前高度大于栈顶高度，出栈，计算出当前墙和栈顶的墙之间水的多少，然后计算当前的高度和新栈的高度的关系，
...
# 第二个是：由于该方法是一行一行计算，在一个凹槽里，利用这个循环可以计算凹槽里的每一行。例如3,2,1,0,3 （3>0 计算第一行，3>1计算第二行，3>2计算第三行）
